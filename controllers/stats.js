...
const stats = {
    createView(request, response) {
        ...
        let largestPlaylist = Math.max(...playlists.map(playlist => playlist.songs.length));
        let longestTitles = playlists.filter(playlist => playlist.songs.length === largestPlaylist)
                            .map(item => item.title);
        ...
        const statistics = {
            ...
            displayLargest: largestPlaylist,
            displayLargestPlaylists: longestTitles,
        };
